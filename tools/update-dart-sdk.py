...
DARTSDK_REPOSITORY = 'https://github.com/dart-lang/sdk'
DARTSDK_REVISION = 'b6592742d9f1a82af319f46eda3d59a61e89b91b'
...
def cleanup():
    ...
    paths = [
        ...
        os.path.join(ROOT_PATH, 'macos', 'third_party', 'dart-sdk')
    ]
    for p in paths:
        if os.path.exists(p):
            shutil.rmtree(p)
...
DARTSDK_FOLDER_README = """# Dynamic linking Dart SDK
...
FAKE_MACOS_SOURCE_HEADER = FAKE_IOS_SOURCE_HEADER
...
def generate(dartsdk_clone):
    ...
    # the original source file. This is necessary because .podspec
    ...
    # Write fake-source files for macos/ which use #include "../..." to include
    # the original source file. This is necessary because .podspec
    # cannot reference sources not under the macos/ folder.
    # But the C-preprocessor can still include them :D
    for f in source_files:
        target = os.path.join(ROOT_PATH, 'macos', 'third_party', 'dart-sdk', f)
        original = os.path.join(ROOT_PATH, 'third_party', 'dart-sdk', f)
        rel = os.path.relpath(original, os.path.dirname(target))
        mkdirp(os.path.dirname(target))
        contents = ''
        contents += FAKE_MACOS_SOURCE_HEADER
        contents += '\n'
        contents += '#include "'+rel+'"\n'
        writeFile(os.path.join('macos', 'third_party', 'dart-sdk', f), contents)
